...
cronset '#alist配置备份' "*/30 * * * * /etc/storage/alist.sh save #alist配置备份"
}
...
alist_time=`nvram get alist_time`
[ ! -z "$alist_time" ] && alist_time=1 && nvram set alist_time=1
[  "$alist_time" -ge "5" ] && alist_time=1 && nvram set alist_time=1
if [ ! -z "\`pidof alist\`" ] ;then
   if [ -s /tmp/alist/data/data.db-wal ] && [ -s /tmp/alist/data/config.json ];then
      ...
      if [ ! -z "$etcsize" ] && [ ! -z "$datasize" ] ; then 
		...
		alist_time=`expr $alist_time + 1`
		nvram set alist_time="$alist_time"
		if [ "$etcsize" -gt "$datasize1" ] ;then	
		 [ "$alist_time" -eq "2" ] && [ "$data1"x = "$data2"x ] && cp -rf /tmp/alist_backup.tgz /etc/storage/alist_backup.tgz && rm -rf /etc/storage/alist/alist_backup.tgz && mv -f /tmp/alist_backup.tgz /etc/storage/alist/alist_backup.tgz && [ -s /etc/storage/alist/alist_backup.tgz ] && logger -t "【AList】" "alist配置$datasize1 k 备份完成，当前/etc/storage可用容量 $etcsize k" && mtd_storage.sh save
		else
