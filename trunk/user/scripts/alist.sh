...
cronset '#alist配置备份' "*/2 * * * * test -z \"\$(pidof alist_save.sh)\" && /etc/storage/alist.sh save #alist配置备份"
}

alist_save () {
if [ ! -s /tmp/alist_save.sh ] ;then
   cat > "/tmp/alist_save.sh" <<-\EEE
#!/bin/sh
datasize="$( du -k /tmp/alist/data/data.db-wal | awk '{print $1}' | tr -d "k" )"
etcsize=`expr $(df -k | grep "% /etc" | awk 'NR==1' | awk -F' ' '{print $4}' | tr -d "M" ) + 0`
upanPath=`nvram get alist_upanPath`
savetime=1
while true; do
savetime=`expr $savetime + 1`
sleep 600
if [ ! -z "\`pidof alist\`" ] ;then
   if [ -s /tmp/alist/data/data.db-wal ] && [ -s /tmp/alist/data/config.json ];then
      cd /tmp/alist
      if [ ! -z "$etcsize" ] && [ ! -z "$datasize" ] ; then  
             [ "$etcsize" -le "$datasize" ] && [ "$savetime" -eq "2" ] && [ -z "$upanPath" ] && logger -t "【AList】" "当前alist配置文件$datasize k 超过了/etc/storage $etcsize k 可用容量，可能无法保存配置"
		...
		[ "$data1"x = "$data2"x ] && cp -rf /tmp/alist_backup.tgz /etc/storage/alist_backup.tgz && rm -rf /etc/storage/alist/alist_backup.tgz && mv -f /tmp/alist_backup.tgz /etc/storage/alist/alist_backup.tgz
		else
		[ "$savetime" -eq "2" ] && logger -t "【AList】" "当前alist备份配置文件包$datasize1 k 超过了/etc/storage $etcsize k 可用容量，无法保存！重启配置可能丢失，可尝试在alist主页进行备份，然后控制台输入rm -rf /tmp/alist/data/data.db-wal 再到alist主页恢复备份来减少配置文件的大小"
	        fi
		rm -rf /tmp/var/data
	     fi
	     else
            [ "$savetime" -eq "2" ] && logger -t "【AList】" "获取alist配置文件$datasize k 和/etc/storage $etcsize k 容量失败，无法备份配置"
      fi
   fi
fi
[ "$savetime" -gt "8" ] && savetime=1
done

EEE
	chmod 755 /tmp/alist_save.sh
fi
if [ ! -z "\`pidof alist\`" ] ; then
/tmp/alist_save.sh &
else
...
if [ -z "$upanPath" ] ; then 
   ...
   logger -t "【AList】" "没有足够空间下载alist,当前/tmp分区只剩$Available_A M，请插U盘使用，或尝试在系统管理-固件升级页面点击扩大/tmp按钮后重新启动脚本，即将退出..."
   exit 1
   fi
 ...
 [ -z "`pidof alist`" ] && logger -t "【AList】" "主程序启动失败, 10 秒后自动尝试重新启动" && sleep 10 && alist_restart
    ...
    nvram set alist_upanPath="$upanPath"
   ...
   if [ ! -s "$alist" ] && [ -s "$upanPath/alist/alist-linux-musl-mipsle.tar.gz" ] ; then
      ...
      logger -t "【AList】" "安装包MD5不匹配，删除重新下载"
      rm -rf  $upanPath/alist/alist-linux-musl-mipsle.tar.gz $upanPath/alist/md5.txt
      fi
   fi
